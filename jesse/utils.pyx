...
from libc.math cimport abs, fmin, fmax, isnan
import numpy as np
cimport numpy as np
np.import_array()
import pandas as pd
...
def estimate_risk(entry_price: float, stop_price: float) -> float:
    ...
    :return: float
    """
    if isnan(entry_price) or isnan(stop_price):
        raise TypeError()
...
def size_to_qty(position_size: float, entry_price: float, precision: int = 3, fee_rate: float = 0) -> float:
    ...
    :return: float
    """
    if isnan(position_size) or isnan(entry_price):
        raise TypeError()
