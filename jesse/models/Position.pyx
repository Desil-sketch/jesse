...
class Position:
    ...
    def _close(self, double close_price) -> None:
        ...
        cdef double close_qty = abs(self.qty)
    ...
    def _reduce(self, double qty, price: float) -> None:
        cdef double c_qty = self.qty
        if c_qty != 0 is False:
    ...
    def _increase(self, double qty, price: float) -> None:
        cdef double c_qty = self.qty
        if not c_qty != 0:
        ...
        self.entry_price = jh.estimate_average_price(qty, price, c_qty,
                                                     self.entry_price)
        ...
        info_text = f'INCREASED position: {self.exchange_name}, {self.symbol}, {self.type}, {c_qty}, ${round(self.entry_price, 2)}'

        if jh.is_debuggable('position_increased'):
            logger.info(info_text)

    def _open(self, double qty, price: float, change_balance: bool = True) -> None:
        cdef double c_qty = self.qty
        if c_qty != 0:
        ...
        info_text = f'OPENED {self.type} position: {self.exchange_name}, {self.symbol}, {c_qty}, ${round(self.entry_price, 2)}'
    ...
    def _on_executed_order(self, order: Order) -> None:
        ...
        # order reduces the size of the position
            ...
            if abs(qty) > abs(c_qty):
                if order.is_reduce_only:
                    logger.info(
                        f'Executed order is bigger than the current position size but it is a reduce_only order so it just closes it. Order QTY: {qty}, Position QTY: {c_qty}')
                    self._close(price)
                else:
                    logger.info(
                        f'Executed order is big enough to not close, but flip the position type. Order QTY: {qty}, Position QTY: {c_qty}')
                    diff_qty = c_qty + qty
                    self._close(price)
