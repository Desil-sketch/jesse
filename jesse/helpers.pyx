...
import numpy as np
cimport numpy as np 
from libc.math cimport abs, floor, log10
...
def hp_to_dna(strategy_hp: list, values: dict) -> str:
    hp = ""
...
def get_config(keys: str, default: Any = None) -> Any:
    ...
    if "pytest" in sys.modules or keys not in CACHED_CONFIG:
        if os.environ.get(keys.upper().replace(".", "_").replace(" ", "_")) is not None:
            ...
            CACHED_CONFIG[keys] = reduce(lambda d, k: d.get(k, default) if type(d) is dict else default,
                                         keys.split("."), config)
...
def is_debuggable(debug_item) -> bool:
    return config['app']['debug_mode'] and config['env']['logging'][debug_item]
...
def is_test_driving() -> bool:
    return False #config['app']['is_test_driving']
...
def now_to_timestamp(force_fresh=False) -> int:
    cdef time_t t 
    t = time(NULL)
    if not force_fresh and (not (is_collecting_data() or is_importing_candles())):
...
def np_ffill(arr, axis: int = 0) -> np.ndarray:
    cdef tuple idx_shape
    cdef np.ndarray idx
...
def orderbook_trim_price(p: float, ascending: bool, unit: float) -> float:
    if ascending:
        ...
        if log10(unit) < 0:
            trimmed = round(trimmed, abs(int(log10(unit))))
...
def should_execute_silently() -> bool:
    return config['app']['trading_mode'] == 'optimize' or "pytest" in sys.modules
